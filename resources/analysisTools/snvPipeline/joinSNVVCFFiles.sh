...
declare -a VCF_FOR_SNV_FILES="$VCF_FOR_SNV_FILES"

# Remove snv files after the merge. There will be remaining hidden checkpoint files for the snv calls (for Roddy)
${PERL_BINARY} ${TOOL_HEADERED_FILE_CONCATENATOR} "${VCF_FOR_SNV_FILES[@]}" \
    | ${BGZIP_BINARY} \
    > ${FILENAME_VCF_RAW}.tmp \
    && mv ${FILENAME_VCF_RAW}.tmp ${FILENAME_VCF_RAW} \
    && ${TABIX_BINARY} -p vcf ${FILENAME_VCF_RAW} \
    && [[ ${keepTemporaryVCFFiles-false} == false ]] \
    && rm "${VCF_FOR_SNV_FILES[@]}"
