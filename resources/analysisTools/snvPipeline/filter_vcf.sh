...
${PERL_BINARY} ${TOOL_SNV_EXTRACTOR} --infile=${FILENAME_VCF} --minconf=${MIN_CONFIDENCE_SCORE} --pid=${outputFilenamePrefix} --bgzip=${BGZIP_BINARY} --tabix=${TABIX_BINARY} --whitelist="${WHITELIST:-NA}" --whitelist_minconf="${WHITELIST_MIN_CONFIDENCE_SCORE:-NA}" --whitelist_maxconf="${WHITELIST_MAX_CONFIDENCE_SCORE:-NA}" ${SNV_FILTER_OPTIONS}
