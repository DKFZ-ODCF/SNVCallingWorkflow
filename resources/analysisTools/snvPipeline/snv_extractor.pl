...
while(<IN>)
{
	...
	next if($_ =~ /^#/);
	my @line = split("\t", $_);
        if($line[$col{"CONFIDENCE"}] >= $whitelist_minconf && $line[$col{"CONFIDENCE"}] <= $whitelist_maxconf ){
# Relaxed settings for genes in Whitelist
	...
	if($line[$col{"ANNOTATION_control"}] eq "somatic" && $line[$col{"RECLASSIFICATION"}] !~ /lowCov_SNP_support_germline/ && $line[$col{"ANNOVAR_FUNCTION"}] !~ /ncRNA/ && ($line[$col{"EXONIC_CLASSIFICATION"}] =~ /nonsynonymous/ || $line[$col{"EXONIC_CLASSIFICATION"}] =~ /stopgain/ ||$line[$col{"EXONIC_CLASSIFICATION"}] =~ /stoploss/ || $line[$col{"ANNOVAR_FUNCTION"}] =~ /splicing/)){
		print COD $_, "\n";
# "Normal somatic coding SNVs" For Whitelist
                push(@{$somatic_functional{$line[$col{"GENE"}]}} ,$_);
...
if ($whitelist ne "NA"){
     #whitelisted and "regular" somatic mutations
     my $outsomwhite = $pid."_somatic_functional_snvs_conf_".$minconf."_to_10_plus_whitelisted_genes_conf_".$whitelist_minconf."_to_".$whitelist_maxconf.".vcf";
...
### Print "regular" functional somatic SNVs to list with whitelisted genes

    foreach $key (keys %somatic_functional)
    {
        @snvs_per_gene=@{$somatic_functional{$key}};
        #print WHITE join("\n",@snvs_per_gene),"\n";
        push @snv_final,@snvs_per_gene;
	...
	if (exists($genes{$line[$whitecol{"gene"}]})){
            ...
            push @snv_final,@snvs_whitelist;
        }
    }    


# Add whitelisted genes to CODing somatic SNVs
     my @filtered = uniq(@snv_final);
